...
st.markdown(
    "Tümör ölçüm değerlerini girerek KNN algoritması ile iyi huylu veya"
    " kötü huylu olma durumunu analiz edin."
)
st.divider()
...
for i, col_name in enumerate(features_list):
  ...
  with target_col:
    payload[col_name] = st.number_input(
        ...
        format="%.4f",
        key=f"cancer_knn_{col_name}",
    )

st.markdown("<br>", unsafe_allow_html=True)
if st.button("KNN ile Teşhisi Başlat 🔍", use_container_width=True, type="primary"):
  try:
    response = requests.post(
        "http://127.0.0.1:8000/predict/cancer_knn", json=payload
    )
    if response.status_code == 200:
      ...
      if prediction == 0:
        ...
        st.error("⚠️ **Sonuç: Kötü Huylu (Malignant)**")
    else:
